                  ...
                  <td className="px-5 py-4">
                    {user.role !== "admin" && (
                      <div className="flex gap-3">
                        <button onClick={() => { setShowResetModal(user); setNewPassword(""); setConfirmPassword(""); setError(""); }} className="text-xs text-blue-400 transition-colors hover:text-blue-300">
                          Đổi mật khẩu
                        </button>
                        <button onClick={() => handleToggleActive(user)} className={`text-xs transition-colors ${user.isActive !== false ? "text-red-400 hover:text-red-300" : "text-green-400 hover:text-green-300"}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-md rounded-2xl border border-zinc-700 bg-zinc-900 p-6">
            ...
            <p className="mb-5 text-sm text-zinc-500">
              Tài khoản: <span className="text-white">{showResetModal.name}</span>
            </p>
            {error && (
              <div className="mb-4 rounded-lg border border-red-500/30 bg-red-500/10 px-4 py-2 text-sm text-red-400">
                {error}
            ...
            <div className="mt-5 flex gap-3">
              <button onClick={() => { setShowResetModal(null); setError(""); }} className="flex-1 rounded-lg bg-zinc-800 py-2.5 text-sm text-zinc-400 transition-colors hover:bg-zinc-700">Hủy</button>
              <button onClick={handleResetPassword} disabled={saving} className="flex-1 rounded-lg bg-amber-500 py-2.5 text-sm font-bold text-zinc-950 transition-colors hover:bg-amber-400 disabled:opacity-60">
