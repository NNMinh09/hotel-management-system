...
import { maintenanceService, locationService, assetService, authService } from "../services";
...
export default function StaffReportForm() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([locationRes, assetRes, technicianRes]) => {
  ...
  return (
    <div className="max-w-xl">
      <div className="mb-6">
        <h1 className="text-2xl font-black text-white">Báo hỏng thiết bị</h1>
        <p className="text-sm text-zinc-500">Điền thông tin và chụp ảnh thực tế</p>
      </div>

      {success && (
        <div className="mb-5 rounded-lg border border-green-500/30 bg-green-500/10 px-4 py-3 text-sm text-green-400">
          Đã gửi yêu cầu thành công!
          {form.assignedTo && " Kỹ thuật viên đã được thông báo."}
        </div>
      )}
      {error && (
      ...
      <form onSubmit={handleSubmit} className="space-y-4 rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
        <div>
          ...
          <input
            ...
            placeholder="Máy lạnh không lạnh, vòi nước bị rò..."
            required
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            placeholder="Mô tả vấn đề, khi nào bắt đầu xảy ra..."
            required
          />
        </div>
        ...
        <div>
          <label className="mb-2 block text-xs uppercase tracking-widest text-zinc-400">Thiết bị (nếu có)</label>
          <select
            value={form.assetId}
        ...
        <div>
          <label className="mb-2 block text-xs uppercase tracking-widest text-zinc-400">
            ...
            <span className="ml-1 normal-case text-zinc-600">(tùy chọn)</span>
          </label>
          <select
          ...
          {form.assignedTo && (
            <p className="mt-1.5 text-xs text-blue-400">
              Phiếu sẽ được giao ngay cho kỹ thuật viên này
            </p>
          )}
        </div>
        ...
        <div>
          <label className="mb-2 block text-xs uppercase tracking-widest text-zinc-400">
            Ảnh chụp thực tế ({images.length}/5)
          </label>
          ...
          {images.length < 5 && (
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className="flex items-center justify-center gap-2 rounded-lg border border-zinc-700 bg-zinc-800 py-3 text-sm text-zinc-300 transition-all hover:border-amber-500 hover:bg-zinc-700"
              >
                Chụp ảnh
              </button>
              <button
          ...
          <p className="mt-2 text-xs text-zinc-600">
            Chụp ảnh thực tế giúp kỹ thuật viên chuẩn bị dụng cụ trước khi đến
          </p>
        </div>
        ...
        >
          {loading ? "Đang gửi..." : "Gửi yêu cầu bảo trì"}
        </button>
      </form>
    </div>
  );
